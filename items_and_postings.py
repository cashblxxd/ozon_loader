...
def get_item_info(product_id, offer_id, shop_api_key, client_id):
	...
	r = requests.post(url="http://api-seller.ozon.ru/v2/product/info", headers=headers, json=payload).json()
	pprint(r)
	r = r["result"]
	...
	result = {
		...
		"Размер НДС, %": {"0.0": "0", "0.1": "10%", "0.2": "20%", "": "0", "0.000000": "0", "0.100000": "10%", "0.200000": "20%", }[str(r.get("vat", "0"))]
	}
	return result
...
def get_items_info(shop_api_key, client_id):
	init_time = time.time()
	print("Getting items...")
	id_list = get_items_ids(shop_api_key, client_id)
...
def dump_items_csv(shop_api_key, client_id, name):
	print("Dumping to", name + "_" + '_products.csv')
...
def dump_postings_csv(shop_api_key, client_id, name):
	print("Dumping to", name + "_" + '_postings.csv')
